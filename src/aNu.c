...
#if !RULE_THEM_ALL && !GTK_CHECK_VERSION(2,90,0)
...
#if !RULE_THEM_ALL && GTK_CHECK_VERSION(2,91,0)
...
#if !RULE_THEM_ALL && GTK_CHECK_VERSION(3,1,90)
...
#endif
#define ANU_PTY_NO_LASTLOG 0
#if !RECORD_LASTLOG
#undef ANU_PTY_NO_LASTLOG
#define ANU_PTY_NO_LASTLOG VTE_PTY_NO_LASTLOG
#endif

#ifndef RECORD_UTMP
#define RECORD_UTMP TRUE
#endif
#define ANU_PTY_NO_UTMP 0
#if !RECORD_UTMP
#undef ANU_PTY_NO_UTMP
#define ANU_PTY_NO_UTMP VTE_PTY_NO_UTMP
#endif

#ifndef RECORD_WTMP
#define RECORD_WTMP TRUE
#endif
#define ANU_PTY_NO_WTMP 0
#if !RECORD_WTMP
#undef ANU_PTY_NO_WTMP
#define ANU_PTY_NO_WTMP VTE_PTY_NO_WTMP
#endif

#define ANU_PTY_NO_HELPER 0
#if !RECORD_LASTLOG && !RECORD_UTMP && !RECORD_WTMP
#undef ANU_PTY_NO_HELPER
#define ANU_PTY_NO_HELPER VTE_PTY_NO_HELPER
#endif
...
#define BACKGROUND_IMAGE ".config/aNu/background.png"
#endif
...
#if (defined(COMMAND_TINT_BG) || defined(BACKGROUND_TINT_COLOR)) && BACKGROUND_EXIST
GdkColor color_tint;
#endif
...
#define ANU_SET_FONT(x,y,z) vte_terminal_set_font_from_string(x,y)
#if defined(FONT_ANTI_ALIAS) || MENU_TOGGLE_ANTI_ALIAS || defined(HOTKEY_TOGGLE_ANTI_ALIAS)
#undef ANU_SET_FONT
// #define ANU_SET_FONT vte_terminal_set_font_from_string_full RM: replaced as 
// per https://developer.gnome.org/vte/0.29/VteTerminal.html#vte-terminal-set-font-from-string-full
#define ANU_SET_FONT vte_terminal_set_font
#endif

#ifndef PROGRAM_NAME
#define PROGRAM_NAME "aNu"
#define UPPER_PROGRAM_NAME "ANu"
#endif

#ifndef PROGRAM_VERSION
#define PROGRAM_VERSION ANU_VERSION
#endif
...
#if COMMAND_TINT_BG
char *command_tint_bg = NULL;
#endif       

#if COMMAND_FG
char *command_fg = NULL;
#endif       
...
void DEL_TAB(GtkWidget *widget, bool do_close_dialog)
{
...
void add_tab()
{
	...
	if (login_shell_flag & 2)
		aNu_argv = default_argv;
	else
#endif
		g_shell_parse_argv(VTE_DEFAULT_COMMAND, NULL, &aNu_argv, NULL);
#if COMMAND_LOGIN_SHELL
	if (login_shell_flag == 1)
		aNu_argv[1] = "-";
#endif
	vte_terminal_fork_command_full(VTE_TERMINAL(term->vte), ANU_PTY_NO_LASTLOG | ANU_PTY_NO_UTMP | ANU_PTY_NO_WTMP | ANU_PTY_NO_HELPER, VTE_DEFAULT_DIRECTORY, aNu_argv, NULL, G_SPAWN_CHILD_INHERITS_STDIN | G_SPAWN_SEARCH_PATH | G_SPAWN_FILE_AND_ARGV_ZERO, NULL, NULL, GET_VTE_CHILD_PID, NULL);
#endif
...
#endif
		GET_VTE_CHILD_PID vte_terminal_fork_command_full(VTE_TERMINAL(term->vte), VTE_DEFAULT_COMMAND, default_argv, NULL, VTE_DEFAULT_DIRECTORY, RECORD_LASTLOG, RECORD_UTMP, RECORD_WTMP);
#endif
...
#if (defined(BACKGROUND_TINT_COLOR) || defined(COMMAND_TINT_BG)) && BACKGROUND_EXIST
...
#if defined(COLOR_BACKGROUND) || defined(COLOR_TEXT_BOLD) || defined(CURSOR_COLOR) || defined(COLOR_TEXT_DIM) || defined(COLOR_FOREGROUND) || defined(COMMAND_FG) || defined(COLOR_TEXT_HIGHLIGHTED)
#ifndef COLOR_STYLE
	...
	GdkColor color_fg;
	if (command_fg) 
		gdk_color_parse(command_fg, &color_fg);
  else
	  gdk_color_parse(COLOR_FOREGROUND, &color_fg);
	vte_terminal_set_color_foreground(VTE_TERMINAL(term->vte), &color_fg);
#endif
...
#ifdef FONT
#if defined(HOTKEY_TOGGLE_ANTI_ALIAS) || MENU_TOGGLE_ANTI_ALIAS
//  ANU_SET_FONT(VTE_TERMINAL(term->vte), font_str, antialias_status); //RM
	ANU_SET_FONT(VTE_TERMINAL(term->vte), pango_font_description_from_string(font_str));
#endif 
#if !defined(HOTKEY_TOGGLE_ANTI_ALIAS) && !MENU_TOGGLE_ANTI_ALIAS
//  ANU_SET_FONT(VTE_TERMINAL(term->vte), font_str, VTE_ANTI_ALIAS);
	ANU_SET_FONT(VTE_TERMINAL(term->vte), pango_font_description_from_string(font_str));
#endif
#endif
...
#if MENU
void set_encoding(GtkWidget *widget, char *data)
{
  GET_CURRENT_TAB(gtk_notebook_get_current_page(GTK_NOTEBOOK(notebook)));
...
int main(int argc, char **argv)
{
	...
	int i = 0;
#endif
#if COMMAND_AT_ROOT_WINDOW || COMMAND_DOCK_MODE || COMMAND_FULLSCREEN || COMMAND_LOGIN_SHELL || PROGRAM_WM_CLASS || COMMAND_SET_TITLE || COMMAND_FONT || COMMAND_TINT_BG || COMMAND_FG || COMMAND_GEOMETRY || COMMAND_SHOW_HELP || COMMAND_SHOW_OPTIONS || COMMAND_SHOW_VERSION || COMMAND_TAB_NUMBERS || RULE_THEM_ALL || defined(MENU_CUSTOM)
	int j = 0;
#endif
...
#if COMMAND_AT_ROOT_WINDOW || COMMAND_DOCK_MODE || COMMAND_LOGIN_SHELL || PROGRAM_WM_CLASS || COMMAND_SET_TITLE || COMMAND_FONT || COMMAND_TINT_BG || COMMAND_FG || COMMAND_GEOMETRY || COMMAND_SHOW_HELP || COMMAND_SHOW_OPTIONS || COMMAND_SHOW_VERSION || COMMAND_TAB_NUMBERS || RULE_THEM_ALL
	j = 1;
...
#if COMMAND_TR
		if (argc > (j + 1) && !strncmp(argv[j], "-tr", 4))
...
#if COMMAND_AT_ROOT_WINDOW || COMMAND_DOCK_MODE || COMMAND_EXEC_PROGRAM || COMMAND_FULLSCREEN || COMMAND_LOGIN_SHELL || COMMAND_SET_TITLE || COMMAND_FONT || COMMAND_TINT_BG || COMMAND_FG || COMMAND_TR || COMMAND_GEOMETRY || COMMAND_SHOW_OPTIONS || COMMAND_SHOW_VERSION || COMMAND_TAB_NUMBERS
			       "s");
#endif
#if !COMMAND_AT_ROOT_WINDOW && !COMMAND_DOCK_MODE && !COMMAND_EXEC_PROGRAM && !COMMAND_FULLSCREEN && !COMMAND_LOGIN_SHELL && !COMMAND_SET_TITLE && !COMMAND_FONT && !COMMAND_TINT_BG && !COMMAND_FG && !COMMAND_TR && !COMMAND_GEOMETRY && !COMMAND_SHOW_OPTIONS && !COMMAND_SHOW_VERSION && !COMMAND_TAB_NUMBERS
			"");
...
#endif
#if COMMAND_TINT_BG
		printf("\t-bg \"[tint] [background]\"   \tspecify background tint\n");
#endif
		
#if COMMAND_FG
		printf("\t-fg \"[foreground]\"   \tspecify foreground color\n");
#endif
		
#if COMMAND_TR
		printf("\t-tr \"[transparency]\"   \tspecify transparency value in [0, 1]\n");
#endif
...
#endif /* COMMAND_AT_ROOT_WINDOW || COMMAND_DOCK_MODE || COMMAND_LOGIN_SHELL || PROGRAM_WM_CLASS || COMMAND_SET_TITLE || COMMAND_FONT || COMMAND_TINT_BG || COMMAND_FG || COMMAND_TR || COMMAND_GEOMETRY || COMMAND_SHOW_HELP || COMMAND_SHOW_OPTIONS || COMMAND_SHOW_VERSION || COMMAND_TAB_NUMBERS || RULE_THEM_ALL */
...
#if RULE_THEM_ALL || GTK_CHECK_VERSION(2,91,7)
...
if (with_gtk == 2)
	...
	gtk_rc_parse_string("style \"aNu\" { GtkButton::default-border = { 0, 0, 0, 0 } GtkButton::default-outside-border = { 0, 0, 0, 0 } GtkButton::inner-border = { 0, 0, 0, 0 } GtkWidget::focus-line-width = 0 xthickness = 0 ythickness = 0 } widget_class \"*.GtkNotebook.GtkHBox.GtkButton\" style \"aNu\"");
#endif
#endif
...
#if defined(COMMAND_TINT_BG) && BACKGROUND_EXIST
gdk_color_parse(command_tint_bg, &color_tint);
#endif
...
if (!gtk_window_set_icon_from_file(GTK_WINDOW(main_window), iconstr, NULL))
	gtk_window_set_icon_from_file(GTK_WINDOW(main_window), ICON_DIR"/aNu.png", NULL);
#endif
